    ...
    <script>
        ...
        function loadContent() {
            ...
            if (currentContent.content_type === 'slideshow') {
                ...
                loadMessage();
            } else if (currentContent.content_type === 'pdf' || currentContent.content_type === 'ppt') {
    loadPDF();  // Handle both types
            } else {
                loadMessage();
            }
        }
...
<<<<<<< HEAD
        ...
        function resolvePdfUrl(contentId, filePath) {
            const base = getAppBasePath();
            const candidates = [];

            // InfinityFree blocks direct /uploads/ PDF access — serve through PHP proxy
            if (contentId) {
                candidates.push(base + 'serve_pdf.php?id=' + contentId + '&mode=' + currentMode);
            }

            let normalized = (filePath || '').replace(/\\/g, '/');
            normalized = normalized.replace(/^\/+/, '').replace(/^uploads\/uploads\//, 'uploads/');
            ...
            if (filename) {
                candidates.push(base + 'uploads/pdf/' + filename);
        ...
        function loadPdfDocument(urls, onSuccess, onError) {
            if (!urls.length) {
                onError(new Error('No PDF URL available'));
                return;
            }

            const tryLoad = (index) => {
                pdfjsLib.getDocument(urls[index]).promise.then(onSuccess).catch(function(error) {

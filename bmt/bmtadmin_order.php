...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_order'])) {
    try {
        ...
        if ($order_data && is_array($order_data)) {
            ...
            foreach ($order_data as $index => $item) {
                $stmt = $pdo->prepare("UPDATE content SET display_order = ? WHERE id = ? AND admin_role = 'bmt'");
            ...
            $stmt = $pdo->prepare("UPDATE content_version SET version = version + 1, last_update = NOW() WHERE admin_role = 'bmt'");
            $stmt->execute();
    ...
    header('Location: bmtadmin_order.php');
    exit();
}
...
if (isset($_GET['toggle_active'])) {
    try {
        ...
        $stmt = $pdo->prepare("SELECT is_active FROM content WHERE id = ? AND admin_role = 'bmt'");
        $stmt->execute([$content_id]);
        $current = $stmt->fetch();

        if ($current) {
            $new_status = $current['is_active'] ? 0 : 1;
            $stmt = $pdo->prepare("UPDATE content SET is_active = ? WHERE id = ? AND admin_role = 'bmt'");
            $stmt->execute([$new_status, $content_id]);

            $stmt = $pdo->prepare("SELECT id, is_active FROM content WHERE admin_role = 'bmt' ORDER BY COALESCE(display_order, 999999) ASC, id ASC");
            $stmt->execute();
...
if (isset($_GET['delete_id'])) {
    try {
        ...
        $stmt = $pdo->prepare("DELETE FROM content WHERE id = ? AND admin_role = 'bmt'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_duration'])) {
    try {
        ...
        $stmt = $pdo->prepare("UPDATE content SET display_duration = ? WHERE id = ? AND admin_role = 'bmt'");
        $stmt->execute([$new_duration, $content_id]);

        $stmt2 = $pdo->prepare("UPDATE content_version SET version = version + 1, last_update = NOW() WHERE admin_role = 'bmt'");
        $stmt2->execute();
...
$stmt = $pdo->prepare("SELECT * FROM content WHERE admin_role = 'bmt' ORDER BY COALESCE(display_order, 999999) ASC, id ASC");
$stmt->execute();
...
<head>
    ...
    <title>BMT Display Order Manager - ET TV</title>
...
<body>
    <div class="header">
        <h1>🎬 BMT Display Order Manager</h1>
        <div class="header-buttons">
            <a href="bmtadmin.php" class="btn btn-primary">➕ Create New Content</a>
        ...
        <div class="order-panel">
            ...
            <div class="order-list" id="orderList">
                <?php if (empty($contents)): ?>
                    <div style="text-align: center; padding: 40px;">No content found. <a href="bmtadmin.php">Create your first display content</a></div>
                <?php else: ?>
    ...
    <script>
        ...
        function displayPreview(content, slides) {
            ...
            // Handle PDF
                ...
                const pdfUrl = content.pdf_proxy_url || '/bmt/pdf_proxy.php?id=' + content.id;

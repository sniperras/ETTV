...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $pdo->prepare("SELECT COALESCE(MAX(display_order), -1) as max_order FROM content WHERE admin_role = 'bmt'");
        $stmt->execute();
        ...
        if ($make_first) {
            $stmt = $pdo->prepare("UPDATE content SET display_order = display_order + 1 WHERE admin_role = 'bmt' ORDER BY display_order DESC");
            $stmt->execute();
            $new_display_order = 0;
        } else if ($insert_position !== null && $insert_position !== '' && $insert_position !== 'last') {
            $insert_pos = (int)$insert_position;
            $stmt = $pdo->prepare("UPDATE content SET display_order = display_order + 1 WHERE admin_role = 'bmt' AND display_order >= ? ORDER BY display_order DESC");
        ...
        if ($content_type === 'slideshow') {
            ...
            if ($layout_type === 'slideshow') {
                ...
                $stmt->execute(['bmt', $content_type, $description, $display_duration, $loop_count, $new_display_order]);
                ...
                $stmt->execute(['bmt', $content_type, $description, $content_data, $display_duration, $loop_count, $new_display_order]);
            }
            ...
            $stmt->execute(['bmt', $content_type, $youtube_link, $display_duration, $loop_count, $new_display_order]);
            ...
            $allowed_files = ['mp4'];
            $file_path = validateAndUploadFile($file, $videosDir, $allowed_files);
            ...
            $stmt = $pdo->prepare("INSERT INTO content (admin_role, content_type, description, content_data, display_duration, loop_count, is_active, display_order) VALUES (?, ?, ?, ?, ?, ?, 1, ?)");
            $stmt->execute(['bmt', 'local_video', $description, $content_data, $display_duration, $loop_count, $new_display_order]);
            ...
            $stmt->execute(['bmt', $content_type, $message_text, $message_type, $display_duration, $loop_count, $new_display_order]);
        }

        // Rebuild the chain after insertion
        $stmt = $pdo->prepare("SELECT id, is_active FROM content WHERE admin_role = 'bmt' ORDER BY display_order ASC, id ASC");
        $stmt->execute();
        $all_content = $stmt->fetchAll();

        $stmt = $pdo->prepare("UPDATE content SET next_content_id = NULL WHERE admin_role = 'bmt'");
        $stmt->execute();
        ...
        $stmt3 = $pdo->prepare("UPDATE content_version SET version = version + 1, last_update = NOW() WHERE admin_role = 'bmt'");
        $stmt3->execute();
        ...
        if ($force_refresh) {
            $stmt_extra = $pdo->prepare("UPDATE content_version SET version = version + 1, last_update = NOW() WHERE admin_role = 'bmt'");
            $stmt_extra->execute();
...
<head>
    ...
    <title>BMT Admin Panel - ET TV</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="header">
        <h1>🎬 BMT Admin Dashboard</h1>
        <div class="header-buttons">
            <a href="../bmt/bmtadmin_order.php" class="header-btn">📋 Manage Display Order</a>
    ...
    <div class="container">
        ...
        <div class="form-card">
            <h2>➕ Create New BMT Display Content</h2>
            <form method="POST" enctype="multipart/form-data" id="contentForm">
                ...
                <div class="form-group" id="layoutTypeGroup" style="display: none;">
                    ...
                    <div class="layout-preview" id="layoutPreview">💡 Selected layout will display all images at once in the chosen arrangement</div>
                </div>

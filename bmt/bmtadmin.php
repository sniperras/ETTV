...
// Check if user is logged in and has bmtadmin role
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt->execute(['bmt', $content_type, $content_data, $display_duration, $loop_count, $next_content_id, $message_type]);

    // Set previous content as inactive
    $stmt2 = $pdo->prepare("UPDATE content SET is_active = 0 WHERE admin_role = 'bmt' AND id != ?");
...
$stmt = $pdo->prepare("SELECT id, content_type, created_at FROM content WHERE admin_role = 'bmt' ORDER BY created_at DESC");
$stmt->execute();
